...
public static partial class Util
{
	...
	public static void SetActiveSchoolUniform(GameObject model, DataManager.CharacterCloth cloth)
	{
		if (SceneManager.GetActiveScene().name != "FPS Room") return;

		// 교복 장구류 탈착
		if (!cloth._Name.StartsWith("School")) return;

		bool useAcc = cloth._Name.EndsWith("Acc");
		Transform acc = model.transform.Find("Acc");
		if (!acc)
		{
			acc = model.transform.Find("Armor");
		}

		if (acc)
		{
			acc.gameObject.SetActive(useAcc);
		}
	}
}